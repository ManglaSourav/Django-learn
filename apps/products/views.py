...
class ProductListView(generics.ListAPIView):
    ...
    """
    queryset = Product.objects.filter(is_active=True, is_deleted=False).select_related('category').prefetch_related('images')
...
class ProductDetailView(generics.RetrieveAPIView):
    ...
    """
    queryset = Product.objects.filter(is_active=True, is_deleted=False).select_related('category').prefetch_related('images', 'variants', 'reviews')
...
class ProductReviewListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        product = get_object_or_404(Product, slug=product_slug, is_active=True, is_deleted=False)
        return ProductReview.objects.filter(product=product, is_approved=True).select_related('user')
...
def related_products(request, product_slug):
    ...
    related = Product.objects.filter(
        ...
        is_active=True,
        is_deleted=False
    ).exclude(id=product.id).select_related('category').prefetch_related('images')[:5]
