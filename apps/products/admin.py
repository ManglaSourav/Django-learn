...
class ProductReviewInline(admin.TabularInline):
    ...
    model = ProductReview
    extra = 0
    fields = ('user', 'rating', 'title', 'comment', 'is_approved', 'is_verified_purchase')
    readonly_fields = ('created_at', 'updated_at')
...
class ProductAdmin(admin.ModelAdmin):
    ...
    Product admin.
    """
    list_display = ('name', 'sku', 'category', 'price', 'stock_quantity', 'is_active', 'is_featured', 'is_low_stock', 'created_at')
    ...
    fieldsets = (
        ('Basic Information', {'fields': ('name', 'slug', 'description', 'short_description', 'sku')}),
        ('Pricing', {'fields': ('price', 'compare_price', 'cost_price')}),
        ('Inventory', {'fields': ('stock_quantity', 'low_stock_threshold', 'weight', 'dimensions')}),
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('category').prefetch_related('images', 'variants', 'reviews')
...
class ProductImageAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Image Information', {'fields': ('product', 'image', 'alt_text', 'is_primary', 'sort_order')}),
        ('Timestamps', {'fields': ('created_at', 'updated_at')}),
    )
...
class ProductVariantAdmin(admin.ModelAdmin):
    ...
    """
    list_display = ('name', 'product', 'sku', 'price', 'stock_quantity', 'is_active', 'created_at')
...
class ProductReviewAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Review Information', {'fields': ('product', 'user', 'rating', 'title', 'comment')}),
